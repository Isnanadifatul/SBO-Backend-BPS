...
async function insertSurveyHandler(request, h) {
  try {
      const {nama_lengkap, jenis_kelamin, umur, pendidikan, masa_kerja, nama_kandidat,
      ...
      // Insert user baru
      await insertUser(nama_lengkap, jenis_kelamin, umur, pendidikan, masa_kerja, nama_kandidat,
        pertanyaan_1, pertanyaan_2, pertanyaan_3, pertanyaan_4, pertanyaan_5, 
        pertanyaan_6, pertanyaan_7, pertanyaan_8, pertanyaan_9, pertanyaan_10,
        pertanyaan_11, pertanyaan_12, pertanyaan_13, pertanyaan_14, pertanyaan_15,
        pertanyaan_16, pertanyaan_17, pertanyaan_18, pertanyaan_19, pertanyaan_20,
...
const AVGConvert30 = async (request, h) => {
  try {
      ...
      for (const result of results) {
          ...
          console.log(`Menyimpan hasil untuk kandidat ${result.nama_kandidat}: ${avgTotal}`);

          if (avgTotal !== undefined) {
              await saveKonversi(result.nama_kandidat, avgTotal);
          } else {
              console.warn(`Nilai untuk ${result.nama_kandidat} adalah undefined`);
          }
      }
...
//
// const handleSaveResponse = async (request, h) => {
//   try {
//       const { nama_kandidat, konversi_nilai } = request.payload;
//       await saveKonversi(nama_kandidat, konversi_nilai);
//       return h.response({ message: 'Response berhasil disimpan' }).code(201);
//   } catch (error) {
//       console.error('Error:', error);
//       return h.response({ error: 'Gagal menyimpan response' }).code(500);
//   }
// };
//get survey tiap kandidat
